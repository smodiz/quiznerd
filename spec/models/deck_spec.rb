...
describe Deck do
  ...
  describe '#search' do
    let(:deck_1) do
      FactoryGirl.create(:deck, name: 'Rails 4', status: 'Public')
    end
    let(:deck_2) do
      FactoryGirl.create(:deck, name: 'Java 4', status: 'Private')
    end
    let(:deck_3) do
      FactoryGirl.create(:deck, name: 'Rails Syntax', status: 'Public')
    end
    let(:deck_4) do
      FactoryGirl.create(:deck, name: 'Something', status: 'Public')
    end
  ...
  describe '#seach_owned_by' do
    ...
    let(:deck_1)  do
      FactoryGirl.create(:deck, name:     'Rails 4',
                                user:     user,
                                status:   'Private',
                                tag_list: 'rails, cool')
    end
    let(:deck_2)  do
      FactoryGirl.create(:deck, name:     'Java 4',
                                user:     user,
                                status:   'Public',
                                tag_list: 'java, cool')
    end
    let(:deck_3)  do
      FactoryGirl.create(:deck, name:     'Rails Syntax',
                                user:     user,
                                tag_list: 'rails')
    end
    let(:deck_4)  do
      FactoryGirl.create(:deck, name:     'Rails Validations',
                                tag_list:  'rails, cool')
    end
  ...
  describe 'with_flash_cards' do
    it 'loads the deck and all associated cards' do
      ...
      FactoryGirl.create(:flash_card, deck_id: deck.id)
