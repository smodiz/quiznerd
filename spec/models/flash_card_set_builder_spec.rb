  ...
  describe '#generate_deck' do
    ...
    context 'with number of cards set to > 0' do
      it 'provides the correct number of flash_cards' do
        ...
        sorted = flash_card_set.map(&:sequence).sort

        expect(flash_card_set.size).to eq 3
        expect(flash_card_set.map(&:sequence)).to eq sorted
      end
    end
    ...
    context 'with a missed filter of :last_missed' do
      it 'returns the flash cards that were missed the last time' do
        missed = deck.flash_cards[0..1].map(&:id).map(&:to_s).join(',')
        FactoryGirl.create(
          :deck_event,
          deck_id: deck.id,
          user_id: deck.user_id,
          missed_cards_list: missed
        )
    ...
    context 'with a missed filter of :ever_missed' do
      it 'returns all flash cards from the given deck that user ever missed' do
        missed_1 = deck.flash_cards[0..1].map(&:id).map(&:to_s).join(',')
        FactoryGirl.create(
          :deck_event,
          deck_id: deck.id,
          user_id: deck.user_id,
          missed_cards_list: missed_1
        )
        missed_2 = deck.flash_cards[1, 2].map(&:id).map(&:to_s).join(',')
        FactoryGirl.create(
          :deck_event,
          deck_id: deck.id,
          user_id: deck.user_id,
          missed_cards_list: missed_2
        )
    ...
    context 'with all options set to non-nil' do
      it 'returns the correct flash cards' do
        missed_cards = deck.flash_cards[0, 4].map(&:id).map(&:to_s).join(',')
        FactoryGirl.create(
          :deck_event,
          deck_id: deck.id,
          user_id: deck.user_id,
          missed_cards_list: missed_cards
        )
    ...
    context 'with options that eliminate everything' do
      it 'returns an empty set' do
        missed_cards = deck.flash_cards[0..1].map(&:id).map(&:to_s).join(',')
        FactoryGirl.create(
          :deck_event,
          deck_id: deck.id,
          user_id: deck.user_id,
