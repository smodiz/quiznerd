...
describe 'Flash Deck Event Pages' do
  ...
  context 'when starting' do
    it 'shows the user options when they select Play' do
      card_count = deck.flash_cards.count * 2
      ...
      expect(page).to have_css('.de-flash-card-side', count: card_count)
    end
  end
  ...
  context 'while playing' do
    ...
    it 'saves the deck_event if the user clicks save', js: true do
      ...
      missed_cards_list = [@first_card.id, @second_card.id].join(',')
      expect(saved_event.total_cards).to eq 2
      expect(saved_event.total_correct).to eq 0
      expect(saved_event.missed_cards_list).to eq missed_cards_list
  ...
  context 'when viewing past flash deck events' do
    before(:each) do
      ...
      @event_3 = FactoryGirl.create(:deck_event, deck_id: deck_3.id,
                                                 user: deck_3.user)
    end

    it 'shows previous deck events for that user and not others' do
      header = 'Flash Card Study History'
      visit deck_events_path

      expect(page).to have_css('.section-header', text: header)
